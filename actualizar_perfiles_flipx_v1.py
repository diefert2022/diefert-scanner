# ============================================================
#  actualizar_perfiles_flipx_v1.py
#  Diefert Scanner v5 — Calibración de FlipX 1-5
#  (variante de actualizar_perfiles_nuevos_v1.py)
# ------------------------------------------------------------
#  QUÉ HACE — igual que la variante de FX Vol/SFX Vol, apuntada
#  a FlipX 1-5. FlipX se queda BIDIRECCIONAL (la dirección la
#  decide la estructura en cada señal, no un lado fijo) — estos
#  datos van a calibrar motor_bidireccional_v1.py, el mismo
#  motor completo (zonas+FVG+OB+CHoCH/BOS) que ya corre para
#  los Vol, solo que ahora aplicado a FlipX.
#
#  CÓMO USARLO: igual que la variante de FX Vol/SFX Vol —
#  MT5 abierto, símbolos visibles en Market Watch, y:
#     python actualizar_perfiles_flipx_v1.py
#
#  Genera perfiles_flipx_actualizados_<fecha>.py — pásamelo y
#  con esos datos calibro FlipX dentro de motor_bidireccional_v1.py.
# ============================================================
...
def generar_archivo_salida(perfiles):
    ...
    nombre_archivo = f"perfiles_flipx_actualizados_{fecha}.py"

    with open(nombre_archivo, "w", encoding="utf-8") as f:
        f.write("# ============================================================\n")
        f.write(f"#  PERFILES FLIPX 1-5 — generado automáticamente\n")
        f.write(f"#  Fecha: {datetime.now().strftime('%Y-%m-%d %H:%M')} UTC\n")
        f.write(f"#  Basado en {MESES_HISTORIAL} meses de historial real (MT5)\n")
        f.write("#\n")
        f.write("#  ⚠️ FlipX se queda BIDIRECCIONAL — el 'sesgo' es solo\n")
        f.write("#  informativo, NO decide una dirección fija. Estos datos\n")
        f.write("#  alimentan motor_bidireccional_v1.py (el mismo motor completo\n")
        f.write("#  que ya usan los Vol, con zonas+FVG+OB+CHoCH/BOS), no el\n")
        f.write("#  motor de dirección fija de PainX/GainX.\n")
        f.write("#\n")
        f.write("#  INSTRUCCIONES:\n")
        f.write("#  Estos bloques NO reemplazan nada automáticamente.\n")
        f.write("#  Pásaselos a Claude para calibrar FlipX dentro de\n")
        f.write("#  motor_bidireccional_v1.py.\n")
        f.write("# ============================================================\n\n")
...
def main():
    print("=" * 60)
    print("  DIEFERT SCANNER — Calibración FlipX 1-5")
    ...
    print("\nListo. Revisa el archivo generado (sobre todo el 'sesgo' de cada uno) y me lo compartes.")
